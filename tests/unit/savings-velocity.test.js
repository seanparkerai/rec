...
export async function register({ test, assert, assertEqual, fixtures }) {
  ...
  await test('savings-velocity: +£500/mo shortens the ETA', () => {
    const v = getSavingsVelocity(finances);
    const plus500 = v.scenarios.find((s) => s.label === '+£500/mo');
    assert(plus500, 'scenario "+£500/mo" missing');
    // A higher monthly contribution must reach the target sooner.
    assert(plus500.deltaMonths > 0, `expected a positive delta (sooner), got ${plus500.deltaMonths}`);
  });
