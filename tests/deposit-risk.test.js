...
const HIGH_RISK_INVESTMENTS = {
  trading212ISA: {
    currentPortfolioValue: 31193,
    earmarkPct: 100,
    strategyEpochs: [
      { id: 'stockpicker', label: 'Individual UK dividend stocks', start: '2025-05-26', end: '2026-01-01' },
      { id: 'etfCore', label: 'ETF + gold core', start: '2026-01-02', end: null },
    ],
  },
};
...
export async function register({ test, assert, assertEqual }) {
  ...
  await test('deposit-risk: high-risk — Luke current state (100% equity ETF, 3-6mo timeline)', () => {
    ...
    assert(r.currentValue === 31193, `expected £31,193 current value, got ${r.currentValue}`);
  });
  ...
  await test('deposit-risk: 10% drop on £31,193 → £28,074', () => {
    const r = assessDepositRisk(HIGH_RISK_INVESTMENTS, GOALS_3_6MO);
    const s10 = r.scenarios.find((s) => s.pctDrop === 10);
    assert(s10 != null, 'no 10% scenario');
    assertEqual(s10.newValue, 28074);
    assertEqual(s10.gapImpact, -3119);
  });
}
