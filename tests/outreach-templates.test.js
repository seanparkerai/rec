...
const SAMPLE_CTX = {
  profile: {
    ...
    postcode: 'SO22 5AA',
    address: '12 Test Street, Winchester, SO22 5AA',
    dob: '1990-01-15',
  },
  finances: {
    income: { annualBaseSalary: 64000, annualBonus: 3000 },
    goal: {
      targetPropertyPrice: 400000,
      offerTarget: 380000,
      targetDeposit: 40000,
      depositPct: 10,
      movingWindow: 'March – September 2026',
    },
    ...
    depositAmount: 40000,
    depositSource: 'Cash ISA — Barclays',
  },
  criteria: {
...
export async function register({ test, assert, assertEqual, fixtures }) {
  ...
  if (renderer) {
    ...
    test('outreach-renderer: estate-agent viewing request (A1) does not leak salary', () => {
      ...
      assert(!body.includes('64000'), 'A1 should not include raw salary figure');
    });
    ...
    test('outreach-renderer: broker enquiry (A5) includes salary in output', () => {
      ...
      const { body } = renderTemplate(tmpl, fullCtx);
      assert(body.includes('64000'), 'A5 should include salary for broker');
    });
