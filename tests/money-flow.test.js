...
export async function register({ test, assert, assertEqual, fixtures }) {
  ...
  await test('money-flow: post-move spare matches finances.spare.monthly at default mortgage £1,900', () => {
    // data/finances.json bakes in spare.monthly = 95 at the £1,900 estimated mortgage.
    const f = getMoneyFlowPostMove(finances, 1900);
