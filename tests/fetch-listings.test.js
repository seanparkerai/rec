...
export async function register({ test, assert, assertEqual }) {
  ...
  test('fetch-listings: dedupeSearchTargets merges targets sharing one identifier', () => {
    const targets = [
      { label: 'GU32:flexcombe-gu32+0', outcode: 'GU32', locationIdentifier: 'POSTCODE^356091', radiusMiles: 3, areas: [{ id: 'flexcombe-gu32' }] },
      { label: 'GU33:flexcombe-gu33+0', outcode: 'GU33', locationIdentifier: 'POSTCODE^356091', radiusMiles: 5, areas: [{ id: 'flexcombe-gu33' }] },
      { label: 'PO14', outcode: 'PO14', locationIdentifier: null, radiusMiles: null, areas: [{ id: 'stubbington-hampshire' }] },
      { label: 'PO15', outcode: 'PO15', locationIdentifier: null, radiusMiles: null, areas: [{ id: 'whiteley-po15' }] },
    ];
    const out = dedupeSearchTargets(targets);
    assertEqual(out.length, 3, 'one duplicate search dropped');
    const merged = out.find((t) => t.locationIdentifier === 'POSTCODE^356091');
  ...
  test('fetch-listings: householdRowsToVillages keeps the full postcode for tight resolution', () => {
    const rows = [{ id: 'stub-hants', data: { name: 'Stub', postcode: 'PO14 2LJ', coords: { lat: 50.8, lng: -1.2 }, coordsSource: 'postcodes-io:places+reverse', source: 'household-onboarding', active: false } }];
