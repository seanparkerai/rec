...
## Phase 8 exit corrections (2026-07-03 — reality wins)

- **ONE profile entry SHIPPED (8.1), superseding the "three-page-profile architecture" prose and
  its "future refactor" proposal above.** `profile.html` now loads exactly one page module:
  `page-profile-page.js` is the page coordinator, importing the three siblings in the legacy
  `<script>` order (page-profile → page-criteria → page-profile-detail; ES import order preserves
  execution order, so behaviour stayed byte-identical). The proposed merge into
  `profile/{about,detail,application}.js` submodules did NOT happen and is not owed — the sibling
  modules stay as-is; only the entry collapsed to meet the §19 one-thin-entry norm. Pinned by
  `tests/pages/profile.test.js`: exactly one `page-*` script on the page + the coordinator's
  import order at source.
- **Module-size inventory above is stale:** current counts are page-profile **249** /
  page-profile-detail **94** / page-profile-page **55** (the coordinator) / page-criteria **381**
  — all inside the §19 ≤400 norm; no split owed.
- **Coupling #6 ("every `dataNeeded` path resolvable") now has a stronger, TWO-WAY rail (8.2):**
  `tests/contract/outreach-templates.test.js` parses every `{{path}}` + `{{#if path}}` from each
  template's subject+body and asserts exact parity with `dataNeeded` in both directions — an
  undeclared used path (silent blank in a real email) and an unused declared path (stale
  over-gating) both fail, with a zero-placeholder guard so a stale parser regex fails loudly.
  The dry run found **zero drift** — the hand-written lists were accurate. Count correction:
  there are **24** templates, not the "30+" claimed in the inventory above.
- **Field-engine branch matrix PINNED (8.4):** `tests/unit/field-engine-branch-matrix.test.js`
  sweeps every buying-situation × employment-basis combination — income/mortgage step inclusion
  (cash-buyer is the ONLY exclusion), the exclusive employed/self-employed work sub-branches,
  earned/pension/joint income fields per combo, the joint-application reveal (`2` and `'2'`
  alike), the FTB-only LISA field, cash-buyer pension hiding, and the consent-gated health notes
  (the consent control itself always visible — withdrawable, UK GDPR). The wizard-era "state
  branching / no undo for hidden fields" risk above is now DOCUMENTED as observed-and-accepted in
  the suite header: hide ≠ purge; autosave writes dirty paths only, so flipping the branch back
  restores prior answers; purge-on-hide would be a deliberate product change, not a bug fix.
- **Outreach-via-Ask a11y AUDITED (8.5) — the pre-fold outreach-page a11y concerns above are
  fully retired.** The Compose dialog was already §11-clean (named native dialog, label-wrapped
  radios/fields, legends, `aria-labelledby` chip group); the drafted message is announced by
  design via the transcript's `aria-live=polite` assistant bubbles plus the draft card's own
  `role=status` region. One gap found and fixed: `#ask-compose-address` (free-text address input)
  had only a placeholder, now `aria-label`'d. Pinned in `tests/pages/ask.test.js`.
- **Mid-phase owner-directed insertion (8.W, GitHub Actions audit)** touched no 10.8 surface —
  recorded at `plan/03-checklist.md` 8.W; its corrections belong to segment 10.10 (CI/workflows).
- **Phase-8 re-baseline:** harness **1097/1097 across 104 suites**; profile/criteria/journey/
  outreach-via-Ask surfaces all pinned (page DOM suites + the two new rails above).
