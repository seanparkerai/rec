...
## §2 scan corrections — Phase-5 reality-wins deltas (appended 2026-07-02 at 5.12 exit)

Phase 5 executed 5.1–5.11 (5.8 decided no-split; 5.9/5.10 absorbed by 3.8c/3.8d). Where this
segment's prose now disagrees with the code, the code wins; the deltas:

- **A5 SHIPPED (5.4) — the "stress test" described at lines ~634–656 no longer exists.**
  `STRESS_UPLIFT_PP` is deleted; the sensitivity rate is `max(assumed rate + RATE_RISE_UPLIFT_PP (1pp),
  RATE_RISE_FLOOR_PCT (7.5%))`, per-household overridable via `finances.mortgage.rateRiseUpliftPP` /
  `rateRiseFloorPct`; output gains `rateRiseRatePct`; every render says "rate-rise sensitivity"
  (widget cell labels `@<rate>%` dynamically). At the household's 5.35% assumed rate the sensitivity
  rate DROPPED 8.35% → 7.50%.
- **A3 reality delta (5.5) — the tile description at 10.2 §Withdrawal-Readiness was doubly stale:**
  the live code had NO LISA logic and no 4–5-year horizon either — it rendered a synthetic
  today→+4-months seasoning bar. Shipped: pure `lisaUsableWindow()` (calc-lisa, §16 extend-only,
  first contribution + 12 months per GOV.UK), a LISA branch in the tile, and `getInvestments()`
  surfacing a `Lifetime ISA` row as `lisa` (primary query now excludes lifetime rows). MCP-confirmed:
  the household has NO LISA row today, so nothing visibly changed; the mechanism arms when one exists.
- **A4 SHIPPED (5.6):** £450k–£500k mismatch line (whyVerdict + deposit banner) + "rules under
  review (2026 consultation)" caveat on every LISA-banner branch + Ask prompt facts. The ask Edge
  Function has NEVER been deployed — repo `prompt.ts` is the sole truth; deploy stays in the owner's
  queued admin handoff.
- **A7 (5.7): items 1–2 SHIPPED** (MGS constants + `bandSignals.mgsEligible` + whyVerdict route line,
  drift-railed; `TRANSACTION_COST_CHECKLIST` + `missingTransactionCosts()` + `#onetime-gaps` footer).
  **Item 3 (leasehold ground rent/service charge as post-move outgoings) remains OPEN** — never in
  5.7's checklist scope; schedule it if/when leasehold targets become real.
- **5.1 correction (already recorded there):** the FTB SDLT cliffs are **£300k/£500k** (Apr-2025
  regime); this file's older £425k/£625k prose refers to the pre-2025 regime.
- **C1/E1 (5.11):** `deriveFinances` warns when `investments` is omitted (explicit null = deliberate);
  onUpdate stays read-once by recorded decision; escaping audit CLEAN (free text only flows through
  the fully-esc'd renderTable; all other writes are numeric formatter output).
- **Net (§5 apparatus):** two golden grids (calc 153 rows + afford 41 rows, regenerated only
  deliberately), constants-drift rail extended to rate-rise + MGS numbers, finance mutation scope
  with ratchet floor `break=86`.
