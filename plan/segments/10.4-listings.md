...
## §2 scan corrections & verified current state (2026-07-01 — reality wins; supersedes stale claims above)

**Verified against the live codebase + live DB. This is the authoritative pre-rewrite snapshot for the flagship segment. Read with `plan/logs/2026-07-01-listings-m2m.md`.**

### The three village-index loaders (confirmed, with the asymmetry that matters)
| Loader | Where | Universe | Radius |
|---|---|---|---|
| `loadOutcodeMap()` | `tools/fetch-listings.mjs:120–139` | **repo-active** areas only (`data/areas/*.json`, `active !== false`) | scalar `geofenceRadiusMi` + `searchRadiusMi`; tuning overlaid later by `applyRadiusTuning()` (:701–728, loaded ~:784) |
| `loadActiveVillages()` | `tools/backfill-geofence.mjs:38–54` | **repo-active** only | scalar only — **no petal/tuning** |
| `restLoadVillages()` | `tools/backfill-listing-areas.mjs:151–192` (+`toVillage()` :51–59) | **DB-canonical** (`areas` table: active OR household-linked incl. stubs) | scalar **or** directional `geofenceRadiiKm`, tuning baked in |

Plus two more partial loaders: `loadAreaCentres()` (`tools/radius-tune.mjs:41–52`, repo areas for bearing math) and the fetcher's `loadRadiusTuning()`. **Critical asymmetry:** the fetcher and backfill-geofence never see household-stub areas; backfill-listing-areas does; and the live feed reads `listing_areas` membership — so junction completeness is load-bearing while its writers disagree about the universe. This is weakness #1 of the 2026-07-01 log, confirmed live.

### Two matchers, both still in the ingestion path (weakness #2 confirmed)
- `matchListingToArea()` — `tools/listings-normalise.mjs:345–364`, nearest-centroid within `IN_OUTCODE_RADIUS_KM = 20` (:21) + address-token fallback; used by `tools/import-apify-runs.mjs:154`.
- `withinGeofence()` — `tools/listings-normalise.mjs:171–210`: nearest by haversine → per-village buffer via `villageBufferKm()` (petal sector by bearing :98–99, scalar fallback 4.8 km ≈3 mi :84) → in-buffer overlap tiebreak preferring the address-named village (:184–192) → `nameAgrees()` corroboration (:194–198) → returns the **full membership set** `areas: [{area_id, distance_mi, is_primary}]` (:202–208).
- `assignArea()` (`tools/fetch-listings.mjs:470–478`) is a third, simpler nearest-only variant inside the fetcher. All three collapse into ONE canonical predicate in the rework.

### Dual primary truth (weakness #3 confirmed)
`listings.area_id` written by fetch-listings (:920), backfill-geofence (:97), backfill-listing-areas drift-fixes (:134–138); `listing_areas.is_primary` written by the RPC writers. Alignment is convention + backfill drift detection — nothing structural prevents divergence.

### The visibility gates, in verified order
1. **Membership** — household's non-origin areas → `listing_areas` → `rightmove_id` filter (`assets/js/storage/listings/feed.js:159–167`).
2. **`geofence_pass IS NOT false`** (feed.js:180–183; belt-and-braces vs membership — weakness #4).
3. **Baseline at source** — type allow-list + price band + min beds (`assets/js/listings/classify.js:57–67`; fetcher-enforced pre-UPSERT).
4. **Search radius** pre-filter (`assets/js/page-listings.js:79–84`; per-area overrides).
5. **Fit hard gates** — affordability out-of-reach (fit.js:79–92), budget floor (:97–110), budget ceiling (:116–129) — `includeOOR` reveals.
6. **Junk classifier** (auction/over-55; flags.js) + **refinement hides** (confirmed overrides) — `includeHidden` reveals (feed-partition.js:59–62).
7. **Decided suppression** by id AND fingerprint — UNCONDITIONAL (feed-partition.js:70).
8. **Fingerprint dedupe** newest-wins, cheapest tiebreak (:71) → controls (search/sort/filter :74–76) → reviewed split (:80–81).

### Dedupe today (weakness #7 audit target)
Three independent layers: per-run `rightmove_id` dedupe (listings-normalise.mjs:366–374); render-time fingerprint dedupe (suppress.js:84–100); durable fingerprint suppression via reactions (suppress.js:32–43). `mergePriceHistory()` appends only on real price change (listings-normalise.mjs:384–396). No cross-source or relist-id merge beyond fingerprints.

### Scheduled fetch (verified)
`.github/workflows/fetch-listings.yml` — 4 slots (08/12/14/18 Europe/London), DST-safe dual crons + once-per-slot dedupe via run history (:188–225); Supabase `pg_cron` dispatch is primary, GH schedule is backstop. Budget: `APIFY_MAX_BUDGET_USD` default $25 (:247), `RESULTS_PER_OUTCODE` 200 (:250), `SEARCH_MODE=cluster` (:248). Demand-gated: only areas ≥1 active household links are fetched; origin areas dropped from the demand set.

### Added since the deep-dive above was written (2026-06-16 → 07-01)
Budget hard-gate ceiling (#25); `listing_areas` m2m + origin exclusion (see log); directional petal radius learning (`refinement/radius*.js`, `tools/radius-tune.mjs`, `area_search_tuning` — 5 rows live); active/inactive household areas + demand-gating; Rightmove added-date display + full membership on cards; the admin `/live-feed/` kiosk (`live-feed/index.html`, `page-live-feed.js`, `live_feed_stats()` RPC, `household_review_stats`). "Report" pages/CSS mentions above are stale — that feature was deleted 2026-06-18.

### Dead code in this path
None found (import-graph audit 2026-07-01): archive tools are intentional one-off records; all live tools referenced by workflows/scripts/docs. The redundancy to kill is **logic** (loaders/matchers/gates), not orphan files.
