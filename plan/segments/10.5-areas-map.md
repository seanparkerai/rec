...
## Phase 6 exit corrections (2026-07-02 — reality wins)

- **Refactor Opportunities §1–3 SHIPPED** as steps 6.1–6.3: matched-price ONE HOME lives at
  `assets/js/areas/matched-price.js` (not affordability.js as proposed — page-layer purity kept);
  priceSummary ONE BAKE via `bakePriceSummary()` in `tools/area-fields.mjs` (derived in sync's
  `canonicalRecord()`, stored DB value ignored — stronger than the §2 proposal, stale-proof);
  `isOnboardingStub()` shared by materialiser + parity gate.
- **§5 content-placeholder / completeness visibility SHIPPED** as 6.4, ONE-HOME-style:
  browser-safe `assets/js/areas/completeness.js` (re-exported by `tools/area-fields.mjs`),
  `researchStatusLine()` rendered as a quiet dossier note. No Web Component — plain template,
  per DESIGN.md.
- **§6 matched-price fallback audit CLOSED at 6.1:** cheapest-available fallback KEPT (recorded in
  the module header) — changing it changes visible verdicts; revisit only with the owner.
- **Content coverage re-baselined:** all 196 areas now `status=researched` with zero missing
  content fields (6.5 batch A: lane-end spot-checked; worthy-down recovered from the cut session's
  surviving DB write; ebble-sp5 honestly recorded as the OS locality label for the Ebble/Chalke
  valley — NOT a settlement, deactivation recommended in `verificationNotes`, owner decision
  pending, mirroring worthy-down's garrison treatment; newtown-sp5 = west-of-Avon Downton.
  6.6 batch B: 16× nearest-school + 5× nearest-pub entries, all web-cited with Ofsted/CAMRA/venue
  sources). Imagery (`images[]`) remains queued in `docs/CHECKLIST.md` — unchanged by Phase 6.
- **The §2 freshness bullet above is stale:** the owed DB→repo materialisation happened 2026-07-02
  (worthy-down recovery); snapshot high-water now tracks the live MAX. Four household-onboarding
  Dorset stubs (added via the portal picker 2026-07-02) are skipped by the materialiser by design.
- **Coord-quality visual cues (§11) and map clustering (§4)** remain dispositioned to the
  post-refactor visual pass (3.7c), unchanged.
