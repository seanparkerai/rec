...
- **The Supabase `areas` mirror is the source of truth (RELAXED §18.5, 2026-06-04 owner decision).**
  ...
  resurrect `docs/Areadetails.md` (now archived under `docs/archive/`).
...
| **Content — `areas`** | `areas` mirror (+ materialised `data/areas/*.json`, index `data/areas.json`) | **Supabase `areas`** (RELAXED §18.5; DB-canonical since 2026-06-04) | Claude writes the DB via MCP → `sync-areas-from-supabase` → `build-areas` → verify/test → commit (one automated write path each way) |
| **Content — other** | `house_types` mirror (+ `data/house-types.json`, `data/checklists.json`, `data/outreach-templates.json` in repo) | **Repo JSON** (git-versioned) | Claude edits the JSON file → mirrors to Supabase content table via MCP UPSERT · portal reads from repo first, falls back to mirror |
...
- **User state**: Supabase always wins. Claude never overwrites a user-edited row unless the user
  ...
  and ask.
- **Content — `areas` (RELAXED 2026-06-04, owner decision):** the Supabase **`areas` mirror is the
  source of truth**; `data/areas/<id>.json` is a materialised view regenerated by
  `tools/sync-areas-from-supabase.mjs`. Every area change is: **write the DB via MCP →
  `sync-areas-from-supabase` → `build-areas` → `verify-area-coords --online` → `run-intelligence-tests`
  → commit.** No actor edits a per-area file by hand as the primary write; if file and DB disagree, the
  DB wins and the file is re-materialised. `data/snapshots/areas.json` + `tests/areas-db-repo-parity.test.js`
  guard this. An id/postcode migration also rewrites `data/source/villages.csv` and carries references
  (incl. the user-state `area_confirmations` keys — the narrow §18.4 relaxation).
- **Content — other (`house_types`, `checklists`, `outreach-templates`)**: Repo JSON still wins. The
  mirror is rebuilt from repo files; the mirror is never edited via the Supabase dashboard. If drift is
  detected, the mirror is re-pushed from repo.
