...
|-------|--------|-----------------|----------------|
| **User state (household)** | `profile`, `criteria`, `finances`, `goals`, `shortlist`, `zones`, `journey_checks`, `contacts`, `outreach`, `readiness_checklist`, `investments_accounts`, `investments_history`, `debts_credit_cards`, `debts_student_loans`, `debts_other` — **NOT in repo JSON files** | **Supabase** (per household_id) | Portal write → Supabase live · Claude write → MCP `execute_sql` UPSERT · `storage.js` mirrors to localStorage for offline reads · `data/fixtures/*.sample.json` provides redacted sample data for tests/fresh-install only |
