...
0. **Supabase freshness check — only if this session edits data, schema, or user-state** (skip entirely
   ...
   blocking ceremony:
   - `mcp__supabase__list_tables` — confirm the 21 tables exist with RLS enabled (inventory in
...
Live schema: **21 tables** (verified via `list_tables` 2026-05-30; +`listing_reactions` added L3) = **16 user-state + 2 content mirrors + 3 system**. Of these, **18 are "tracked"** for the sync contract (16 user-state + 2 content); the 3 system tables are Supabase-managed and never synced by Claude. This 16+2+3 split is the single source of truth — see `docs/SUPABASE_SYNC.md`; §8's count must match it. (`listing_reactions` is user-state but **append-only**; the fetcher-written `listings` table is live-content and is **not** tracked.)

| Class | Tables | Source of truth | Sync direction |
|-------|--------|-----------------|----------------|
| **User state (household)** | `profile`, `criteria`, `finances`, `goals`, `shortlist`, `zones`, `journey_checks`, `contacts`, `outreach`, `readiness_checklist`, `investments_accounts`, `investments_history`, `debts_credit_cards`, `debts_student_loans`, `debts_other`, `listing_reactions` (append-only) — **NOT in repo JSON files** | **Supabase** (per household_id) | Portal write → Supabase live · Claude write → MCP `execute_sql` UPSERT (INSERT for append-only `listing_reactions`) · `storage.js` mirrors to localStorage for offline reads · `data/fixtures/*.sample.json` provides redacted sample data for tests/fresh-install only |
