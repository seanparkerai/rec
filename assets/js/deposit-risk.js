...
function deriveEquityPct(isa) {
  ...
  // etfCore epoch = diversified equity ETFs + gold.
...
function buildRecommendation(verdict, timelineMonthsMax, currentValue, hopedDeposit) {
  if (verdict === 'high-risk') {
    return {
      ...
      urgency: 'high',
      reasoning: `A ${timelineMonthsMax}-month timeline combined with a high equity allocation means a market correction directly reduces your deposit. Move the funds into your deposit account at least 3 months before applying so lenders can see a clean source trail.`,
    };
  }
