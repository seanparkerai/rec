...
/**
 * Ordered, de-duplicated floor-plan image URLs for a listing — empty when the
 * source never carried one (the dossier then omits the section entirely, so the
 * floor plan shows only "if available"). Reads, in order: a typed `floorplan_url`
 * column, then a `raw_json.floorplans` array (each entry a { url } / { src }
 * object or a bare URL string), then a single `raw_json.floorplan`. Floor-plan
 * data only exists on detail-page scrapes; the search-summary fetch carries none.
 * @param {object} listing  a listings row (with raw_json)
 * @returns {string[]}
 */
export function floorplanImages(listing) {
  const out = [];
  const seen = new Set();
  const push = (u) => { if (isHttp(u) && !seen.has(u)) { seen.add(u); out.push(u); } };
  push(listing?.floorplan_url);
  const fps = listing?.raw_json?.floorplans ?? listing?.raw_json?.floorplan;
  if (Array.isArray(fps)) for (const f of fps) push(typeof f === 'string' ? f : (f?.url ?? f?.src));
  else if (fps) push(typeof fps === 'string' ? fps : (fps?.url ?? fps?.src));
  return out;
}
