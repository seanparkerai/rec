...
- `tests/affordability.test.js` *(new)* — hand-computed cases:
  - £300k house on default finances → `comfortable`
  - £380k → `stretch`
  - £420k → `tight`
  - £500k → `out-of-reach`
  - £449k → `lisaEligible: true`; £451k → `false`
  - high stressed rate → `whyVerdict` flags stress test
...
**Acceptance**: all new tests green; verdict for default `finances.json` against `targetPropertyPrice = £380k` is `stretch`. Commit: `feat: phase 2 — intelligence engine (affordability, money-flow, savings-velocity, format)`.
...
**Tiles, in order**:
1. **Lede strip** (full width, ~180 px) — single editorial line synthesised from criteria ("3-bed in Hampshire & Wiltshire around £380k · 14 months from completing your deposit"); right-side 4-mono-numeral mini-strip from existing lede.
...
3. **Affordability verdict** (1/3 right) — horizontal price ladder (£250k–£500k), banded comfortable/stretch/tight/out-of-reach, marker at `offerTarget`. Single-sentence verdict in display font. Two **ephemeral** "what about £X?" inputs that slide the marker. *(No persistence — confirmed.)*
4. **Money-flow** (full width) — side-by-side **today vs after-move** flows. Pure SVG (stacked horizontal bars, segment widths proportional to monthly amounts; inline mono labels). The "after" panel highlights the new mortgage segment and shrunk spare. Caption: "Spare drops from £1,995 to £95/mo." Tapping a segment expands its line items in a `<details>` below.
...
- `pages/ask.html` (~80 lines) — disabled NL input + suggestion-chip examples (`"chain-free 3-beds near Winchester under £400k"`, etc.).
